...
import { ZimmoScraper } from "./src/scrapers/zimmo.js";
...
async function test() {
  ...
  const scraper = new ZimmoScraper(logger);
  const browser = await chromium.launch({ headless: true });
  const page = await browser.newPage();

  try {
    const url = scraper.buildSearchUrl({ city: "Gent" });
    console.log(`=== Testing Zimmo: ${url} ===\n`);
