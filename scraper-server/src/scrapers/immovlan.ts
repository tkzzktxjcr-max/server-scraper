...
export class ImmovlanScraper extends BaseScraper {
  constructor(siteId: string, filters: ScraperFilters, jobLogger: JobLogger) {
    super("immovlan", siteId, filters, jobLogger);
  }

  protected getApiPattern(): string | RegExp {
    return /immovlan\.be\/api|immovlan\.be\/search\/api|api\.immovlan/;
  }

  protected buildSearchUrl(page: number): string {
    let baseUrl = "https://www.immovlan.be/en/search";
  ...
  protected async parseSearchResults(page: Page, apiResponses: InterceptedResponse[]): Promise<SearchResultItem[]> {
    ...
    // ─────────────────────────────────────────────
    // STRATEGY 2: Extract from page JSON / embedded data
    // ─────────────────────────────────────────────
        ...
        const embeddedData = await page.evaluate(() => {
          ...
          for (const script of scripts) {
            try {
              ...
              if (data.results || data.properties || data.listings) {
    ...
    if (items.length === 0) {
      ...
      try {
        const cssItems = await page.evaluate(() => {
          const cards = document.querySelectorAll('a[href*="/property/"], a[href*="/detail/"], [class*="property-card"], [class*="listing-item"]');
          return Array.from(cards).map((card) => {
            const link = card.tagName === "A" ? card as HTMLAnchorElement : card.querySelector("a") as HTMLAnchorElement;
            if (!link) return null;
            ...
            const priceEl = card.querySelector("[class*='price'], [class*='value']");
  ...
  private parseApiListItem(item: Record<string, unknown>): SearchResultItem | null {
    try {
      const source_id = cleanString(item.id || item.propertyId || item.reference || "");
      if (!source_id) return null;

      let url = cleanString(item.url || item.link || item.detailUrl || "");
      if (!url) {
        url = `https://www.immovlan.be/en/property/${source_id}`;
      }
      if (!url.startsWith("http")) {
        url = `https://www.immovlan.be${url.startsWith("/") ? "" : "/"}${url}`;
      }
  ...
  private parseDetailData(data: Record<string, unknown>, item: SearchResultItem): RawListing | null {
    try {
      ...
      const source_id = cleanString(data.id || data.reference || item.source_id);
      ...
      this.jobLogger.error("Failed to parse Immovlan detail data", {
        error: error instanceof Error ? error.message : String(error),
      });
      return null;
    }
  ...
  private async extractDetailFromCSS(page: Page, item: SearchResultItem): Promise<RawListing | null> {
    try {
      const data = await page.evaluate(() => {
        ...
        const priceText = document.querySelector("[class*='price'], [class*='value']")?.textContent || "";
        ...
        document.querySelectorAll("dt, th, [class*='label'], [class*='feature-label']").forEach((el) => {
        ...
        const photos: string[] = [];
        document.querySelectorAll("[class*='gallery'] img, [class*='carousel'] img").forEach((img) => {
      ...
      this.jobLogger.error("Immovlan CSS detail extraction failed", {
        error: error instanceof Error ? error.message : String(error),
      });
      return null;
    }
